...
class EndoMapperParser:
    def __init__(self, input_folder):
        ...
        self.depth_paths = sorted(glob.glob(f"{self.input_folder}/depth/*.exr"))

        self.load_poses(f"{self.input_folder}/trajectory.csv")
    ...
    def load_poses(self, path):
        ...
        def get_transformation_matrix(t, r):
            rotation = R.from_quat(r).as_matrix()
        ...
        for i, row in df.iterrows():
            ...
            r = [row['rX'], row['rY'], row['rZ'], row['rW']]

            try:
                pose = get_transformation_matrix(t, r)
...
class WildParser:
    def __init__(self, input_folder, scale=1.0):
        self.input_folder = input_folder
        self.color_paths = sorted(glob.glob(f'{self.input_folder}/images-small/*.jpg'))
...
class C3VDParser:
    def __init__(self, input_folder, depth='cv3d', scale=100.0, long=False):
        self.input_folder = input_folder
        self.color_paths = \
            sorted(glob.glob(f'{self.input_folder}/pps/images/*.png'))

        if depth == 'c3vd':
            self.depth_paths = sorted(glob.glob(f"{self.input_folder}/depths-small/*.tiff"))
        elif depth == 'c3vd-m':
            self.depth_paths = sorted(glob.glob(f"{self.input_folder}/depths-marigold/*.png"))
        elif depth == 'c3vd-mv':
            self.depth_paths = sorted(glob.glob(f"{self.input_folder}/depths-marigold-video-384/*.tiff"))
        elif depth == 'dav2':
            self.depth_paths = sorted(glob.glob(f"{self.input_folder}/depths-DAv2/*.png"))
        elif depth == 'pps':
            self.depth_paths = sorted(glob.glob(f'{self.input_folder}/pps/depths/*.png'))
        elif depth == 'dpt':
            self.depth_paths = sorted(glob.glob(f'{self.input_folder}/dpt-hybrid-384/*.png'))
        ...
        self.n_img = len(self.color_paths)
        try:
            self.load_poses(f"{self.input_folder}/poses_gt.txt")
        except:
            self.load_poses_2(f"{self.input_folder}/pose.txt")

        if long:
            leng = 4000
            self.color_paths = self.color_paths[:leng]
            self.depth_paths = self.depth_paths[:leng]
            self.n_img = leng
...
class Colon10kParser:
    def __init__(self, input_folder, scale=1.0):
        # /playpen-nas-ssd/asdunnbe/med_slam/real_data/UNC_Colon10K/20/
        self.input_folder = input_folder
        self.color_paths = \
            sorted(glob.glob(f'{self.input_folder}/pps/images/*.png'))
        try:
        ...
        self.color_paths = self.color_paths[::2]
        self.depth_paths = self.depth_paths[::2]

        self.scale = scale
        self.n_img = len(self.color_paths)
        self.load_poses(self.n_img)

    def load_poses(self, length):
        self.poses = []

        frames = []
        for i in range(length):
            c2w = np.eye(4)
            c2w[:3, 3] = i* .1 * self.scale
            w2c = np.linalg.inv(c2w)
            self.poses.append(w2c)
            frame = {
                "file_path": self.color_paths[i],
...
class EndoMapperDataset(MonocularDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
...
class C3VDDataset(MonocularDataset):
    def __init__(self, args, path, config, long=False):
        super().__init__(args, path, config)
        depth = config["Dataset"]["depth"]
        dataset_path = config["Dataset"]["dataset_path"]

        scale = config['Dataset'].get('scale', 1.)

        parser = C3VDParser(dataset_path, depth, scale, long)
...
class WildDataset(MonocularDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        dataset_path = config["Dataset"]["dataset_path"]

        scale = config['Dataset'].get('scale', 1.0)

        parser = WildParser(dataset_path, scale)
...
class Colon10kDataset(MonocularDataset):
    def __init__(self, args, path, config):
        super().__init__(args, path, config)
        dataset_path = config["Dataset"]["dataset_path"]
        ...
        parser = Colon10kParser(dataset_path, scale)
...
def load_dataset(args, path, config):
    ...
    elif config["Dataset"]["type"] == "c3vd":
        return C3VDDataset(args, path, config)
    elif config["Dataset"]["type"] == "c3vd-long":
        return C3VDDataset(args, path, config, long=True)
    elif config["Dataset"]["type"] == "colon10k":
        return Colon10kDataset(args, path, config)
    elif config["Dataset"]["type"] == "endomapper":
        return EndoMapperDataset(args, path, config)
    elif config["Dataset"]["type"] == "euroc":
        return EurocDataset(args, path, config)    
    elif config["Dataset"]["type"] == "wild":
        return WildDataset(args, path, config)
